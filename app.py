...
from threading import Thread
    ...
    # Load the cascade
face_cascade = cv2.CascadeClassifier('saved_model/model.xml')
...
def detect_face(frame):
    ...
    # Draw the rectangle around each face
        ...
        # color = (255, 0, 0)
        # frame = cv2.rectangle(frame, (startY,endY), (startX,endX),color,2)
    ...
    if request.method == 'POST':
        if request.form.get('click') == 'Capture':
            ...
            if(switch == 1):
                ...
                camera = cv2.VideoCapture(0)
                switch = 1
        elif request.form.get('rec') == 'Start or Stop':
            global rec, out
            rec = not rec
            if(rec):
                now = datetime.datetime.now()
